	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-xs-12 lead">
				<ol class="breadcrumb">
				  <li class="active">Listado de calificaciones</li>
				  <li><a href="nuevo.php">Nueva calificación</a></li>
				</ol>
			</div>
		</div>
	</div>
...
?>
	...
	<div class="container-fluid">
            <h2 class="text-center all-tittles">listado de calificaciones</h2>
		  ...
		  <input id="FiltrarContenido" type="text" class="form-control" placeholder="Buscar periodos..." aria-label="Grado" aria-describedby="basic-addon1">
		</div>
			...
			$db = $database->open();
			try{	
				$sql = 'SELECT cali.codcali, anoaca.nomano, periodo.nompe, docente.nomprof, clase.nomclase, seccion.nomsec, alumnos.nombrea,cali.praccali, cali.examparc, cali.indiv, cali.fechare  FROM cali INNER JOIN anoaca ON cali.codano= anoaca.codano INNER JOIN periodo ON cali.codper = periodo.codper INNER JOIN docente ON cali.coddoce= docente.coddoce INNER JOIN clase ON cali.codclase= clase.codclase INNER JOIN seccion ON cali.codsec = seccion.codsec INNER JOIN alumnos ON cali.codalum= alumnos.codalum';
				foreach ($db->query($sql) as $row) {
		
	//	$total_score = $row['test1'] + $row['test2'] + $row['test3'];
		$total_score = ($row['praccali']*0.20) + ($row['indiv']*0.20) + ($row['examparc']*0.60);
						$cal=$total_score/300;
						$ave=$cal*100;
					...
					<tr>
						...
						<td><?php echo $row['nompe']; ?></td>
					<!-- ------------------------------------------>
					
					
						<td><?php echo $row['praccali']; ?></td>
						<td><?php echo $row['examparc']; ?></td>
						<td><?php echo $row['indiv']; ?></td>
						<td><?php echo number_format($total_score, 1).""?></td>
						<td><?php 
						if($total_score<10.5)
							echo " <span class='label label-danger'> Desaprobado";
						else 
							echo " <span class='label label-success'> Aprobado";
						?></td>
						
						<td>
						
							
							<a href="#ver_<?php echo $row["codcali"]; ?>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></a>
								
								
								
								<a href="#delete_<?php echo $row["codcali"]; ?>" style="display:none;" title="Eliminar" data-backdrop="false" class="btn btn-danger btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
