...
$display_heading = array('codsec'=>'#', 'nomsec'=> 'Salón', 'capacidad'=> 'Capacidad');

$result = mysqli_query($connString, "SELECT seccion.codsec, seccion.nomsec, seccion.capacidad,clase.nomclase,docente.nomprof, seccion.fecha_create  FROM seccion INNER JOIN clase ON seccion.codclase = clase.codclase INNER JOIN docente ON seccion.coddoce = docente.coddoce") or die("database error:". mysqli_error($connString));
...
{
$pdf->Cell($w[0],6,$row['codsec'],1);
$pdf->Cell($w[1],6,utf8_decode($row['nomsec']),1);
$pdf->Cell($w[2],6,utf8_decode($row['capacidad']),1);
